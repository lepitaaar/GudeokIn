...
export const getUserByUUID = async (
    ...
    const userData = await db.query(
        `SELECT uuid, grade, class, number, role, profileImage, nickname, email, nickchangeDate, isNeisSync, neisSyncDate, credit, isCmAr from everytime.user_info where uuid = @uuid`,
        {
            uuid: uuid,
        }
    );
